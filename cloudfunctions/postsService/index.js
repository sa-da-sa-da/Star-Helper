...
async function addPostComment(event) {
  ...
  if (process.env.author == event.commentContent.cOpenId) {
   ...
   let task = db.collection('Article_List').doc(event.commentContent.postId).update({
     data: {
      Article_Comment: _.inc(1)
     }
  });
...
async function addPostChildComment(event) {
  ...
  let task = db.collection('Article_List').doc(event.postId).update({
    data: {
      Article_Comment: _.inc(1)
    }
  });
